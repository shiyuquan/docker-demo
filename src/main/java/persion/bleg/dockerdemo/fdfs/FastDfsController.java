...
import persion.bleg.dockerdemo.minio.Chunk;
import persion.bleg.dockerdemo.minio.MinioConfig;
import persion.bleg.dockerdemo.minio.my.MyMinioClient;
import persion.bleg.dockerdemo.util.Files;
...
@Api(tags = "fdfs 接口")
@RestController
@RequestMapping(DEFAULT_API_PREFIX + "/fdfs")
public class FastDfsController {
     ...
     */
    @ApiOperation(value = "完成分片文件上传")
    @PostMapping("simpleUploader/chunk/complete-minio")
    public void dowloadMinio(@RequestParam("md5") String md5, HttpServletResponse response) {
        long start = System.currentTimeMillis();
        ...
        // 创建文件夹
        log.info("创建文件夹{}", tempFolder);
        Files.newFolder(tempFolder);
        List<String> chunkPath = new ArrayList<>();
        try {
            for (int i = 0; i < 205; i++) {
                tempFileName = i + "-" + fileName + ".tmp";
                try {
                    log.info("下载分片 {}", tempFileName);
                    InputStream fileInputStream = minioClient.getObject(minioConfig.getBucketName(), tempFileName);
                    log.info("生成本地分片 {}", tempFileName);
                    Files.saveFile(fileInputStream, tempFolder, tempFileName);
                } catch (Exception e) {
                    throw new BlegException(500, "下载失败");
                }
                chunkPath.add(tempFolder + File.separator + tempFileName);
            }

            // 合并分片, 将文件保存到本地
            log.info("合并分片, 将文件 {} 保存到本地", tempFolder + File.separator + fileName);
            Files.merge(tempFolder + File.separator + fileName, chunkPath);

            // 下载本地文件
            log.info("下载本地文件 {}", tempFolder + File.separator + fileName);
            FileInputStream fis = new FileInputStream(new File(tempFolder + File.separator + fileName));
            IOUtils.copy(fis, response.getOutputStream());
